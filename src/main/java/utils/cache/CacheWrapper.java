...
public final class CacheWrapper<K, V> {
    ...
    public static <K, V> Cache<K, V> newCache(String cacheName) {
        Cache<K, V> cache = CacheBuilder.newBuilder().build();
        putIfAbsent(cacheName, cache);
        return cache;
    }

    public static <K, V> Cache<K, V> newCache(String cacheName, String config) {
        Cache<K, V> cache = CacheBuilder.from(CacheBuilderSpec.parse(config)).build();
        putIfAbsent(cacheName, cache);
        return cache;
    }

    public static <K, V> Cache<K, V> getOrDefault(String cacheName) {
        Cache<K, V> cache = (Cache<K, V>) getInstance().cacheMap.getOrDefault(cacheName, CacheBuilder.newBuilder().build());
        putIfAbsent(cacheName, cache);
        return cache;
    }

    public static <K, V> Cache<K, V> getOrDefault(String cacheName, String config) {
    ...
    public static <K, V> void putIfAbsent(String cacheName, Cache<K, V> cache) {
